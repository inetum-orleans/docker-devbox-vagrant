...
EOF
  ...
  sudo apt-get install -y autoconf dpkg-dev file g++ gcc libc-dev make pkg-config re2c libonig-dev libreadline-dev \
    libxml2-dev libcurl4-openssl-dev libjpeg-dev libpng-dev libtidy-dev libxslt1-dev libzip-dev libbz2-dev \
    libssl-dev libmcrypt-dev libkrb5-dev libpq-dev libsqlite3-dev libmysqlclient-dev unzip
